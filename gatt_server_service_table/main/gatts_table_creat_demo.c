...
static esp_ble_adv_data_t adv_data = {
    ...
    .min_interval        = 0x00060, //slave connection min interval, Time = min_interval * 1.25 msec
    .max_interval        = 0x00100, //slave connection max interval, Time = max_interval * 1.25 msec
    .appearance          = 0x00,
...
static esp_ble_adv_data_t scan_rsp_data = {
    ...
    .include_name        = true,
    .include_txpower     = true,
    .min_interval        = 0x00060,
    .max_interval        = 0x00100,
    .appearance          = 0x00,
    .manufacturer_len    = 0, //TEST_MANUFACTURER_DATA_LEN,
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            if (param->add_attr_tab.status != ESP_GATT_OK){
                ...
                for (int i = 0; i < HRS_IDX_NB; i++) {
                    ESP_LOGI(GATTS_TABLE_TAG, "%04x", heart_rate_handle_table[i]);
                }
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        ESP_LOGI(GATTS_TABLE_TAG, "Reg event handling, app_id %04x", param->reg.app_id);
